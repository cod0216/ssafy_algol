...
public class Main {
	...
	public static void canGo(int dep, int location[]) {
		if(distance(location, rock) <= 1000) {
			answer = "happy";
			return;
		}
		...
		for(int i=0 ; i < N ; i++) {
			if(!visit[i] && distance(location, stores[i]) <= 1000) {
				visit[i] = true;
				canGo(dep+1, stores[i]);
			}
		}
	}
