...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (M-- > 0) {
            ...
            else if (possible[bead]) sb.append("Y ");
            else sb.append("N ");
        }
    ...
    static void dfs(int index, int diff) {
        if (index > N || dp[index][diff]) return;
        dp[index][diff] = true;
        possible[diff] = true;

        if (index == N) return;

        int w = weight[index + 1];
        dfs(index + 1, diff); // 추 사용 안함
        dfs(index + 1, diff + w); // 추 왼쪽에 추가
        dfs(index + 1, Math.abs(diff - w)); //오른쪽에 추가
    }
}
